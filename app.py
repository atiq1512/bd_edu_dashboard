...
students_with_intention = len(
df[df["IntentionStatus"] == 1]
)

students_without_intention = len(
df[df["IntentionStatus"] == 0]
)
...
col2.metric(
"With Intention",
students_with_intention
)

col3.metric(
"Without Intention",
students_without_intention
)
...
# ENTREPRENEURIAL INTENTION DISTRIBUTION
...
```
st.subheader(
    "Trait Comparison by Entrepreneurial Intention"
)
...
corr = df[
    available_traits + ["IntentionStatus"]
].corr()
...
st.success(
f"{intention_rate:.2f}% of students demonstrate entrepreneurial intention."
)
